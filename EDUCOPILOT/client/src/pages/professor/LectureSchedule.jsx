...
const LectureSchedule = () => {
  ...
  // Active Import Tab: 'file' | 'calendar' | 'manual'
  const [activeTab, setActiveTab] = useState('file');

  // Multi-modal upload state
  const [importFile, setImportFile] = useState(null);
  const [defaultSubject, setDefaultSubject] = useState('Computer Science');
  const [defaultCourseCode, setDefaultCourseCode] = useState('CS-301');
  const [rawText, setRawText] = useState('');

  // Interactive Calendar / Manual Picker State
  const [manualSubject, setManualSubject] = useState('Computer Science');
  const [manualCourseCode, setManualCourseCode] = useState('CS-301');
  ...
  // Handle Multi-Modal Timetable Import (PDF, Excel, Image, Text)
  const handleImportTimetable = async (e) => {
    ...
    try {
      ...
      if (extracted.length === 0) {
        ...
        setSuccessMsg(`Extracted ${extracted.length} lecture sessions into the staging table below. Review and edit before saving!`);
      }
    } catch (err) {
  ...
  // Add a manual item to the staging table
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white flex items-center gap-2.5">
          ...
          <span>Lecture Scheduling & Timetable Import</span>
        </h1>
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
          Import course schedules from PDF syllabi, Excel/CSV sheets, Timetable photos (OCR), or interactive calendar entry. Edit extracted rows in the staging area and save directly to your course calendar.
        </p>
      </div>
      ...
      <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Calendar Entry & Manual Typing</span>
            </button>
          </div>
        </div>

        {/* Tab 1: Multi-Modal File / Image / Text Import */}
        {activeTab === 'file' && (
          <form onSubmit={handleImportTimetable} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="e.g. Mon 10:00 AM - Section A: CAP Theorem & Consistency Models..."
        ...
        {/* Tab 2: Calendar Slot & Manual Entry */}
        {activeTab === 'manual' && (
          <form onSubmit={handleAddManualToStaging} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Distributed Consensus: Raft Protocol & Quorum Invariants"
                className="w-full px-3.5 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-600 focus:outline-none dark:text-white"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Leader Election, AppendEntries, Log Invariants"
                className="w-full px-3.5 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-600 focus:outline-none dark:text-white"
              />
            </div>
                ...
                {stagedLectures.map((lec, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-2 px-3">
                      <input
                        type="text"
                        value={`${lec.subject || 'CS'} (${lec.courseCode || 'CS-301'})`}
                        onChange={(e) => handleUpdateStagedField(idx, 'subject', e.target.value)}
                        className="w-28 px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs dark:text-white"
                      />
                    </td>
                    <td className="py-2 px-3">
                      <input
      ...
      <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm space-y-4">
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-xs text-slate-400 dark:text-slate-500">
            No lectures scheduled yet. Use the multi-modal import above to add your timetable.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {schedules.map((lec) => (
              <div
                key={lec._id}
                className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/70 flex flex-col justify-between gap-3 group hover:border-blue-500/50 transition-all"
              >
                <div className="space-y-2">
                  ...
                  {lec.topics && lec.topics.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {lec.topics.slice(0, 3).map((t, i) => (
                        <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-slate-200/60 dark:bg-slate-700/60 text-slate-600 dark:text-slate-400">
                          {t}
                        </span>
                      ))}
                    </div>
